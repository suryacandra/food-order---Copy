...
const Cart = (props) => {
    ...
    return (
        <div>
            <div onClick={props.on} className="absolute w-full h-[850px] bg-slate-800/60 p-5">
            </div>
            <div className="grid grid-cols-2 w-[414px] sm:w-1/2 place-items-center gap-2 bg-white rounded-lg absolute p-5 m-3 top-[30%] left-[0%] sm:left-[25%] sm:right-[100%]">
                ...
                {food.items.length > 0 && <button className="bg-green-300 w-1/2 rounded-lg p-2 px-5" onClick={props.order}>Order</button>}
                </>}
                {props.status && <div className="col-span-2">
                    <Order on={props.on} status={props.status} order={props.order} food={food} />
                </div>}
            </div>
        </div>
    )
...
const CartList = props => {
    ...
    return (
        <>
            {createPortal(<Cart order={orderHandler} status={order} {...props} />, document.getElementById('cart'))}
        </>
    )
}
